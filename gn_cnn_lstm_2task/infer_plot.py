...
def load_model(ckpt_path: Path):
    ...
    gn_groups = int(config.get("gn_groups", 8))
    state = ckpt.get("model_state", ckpt)
    model = GNCNNLSTM2Head(input_dim=N_MELS, cnn_channels=64, hidden=128, bidirectional=bilstm, gn_groups=gn_groups)
    try:
        model.load_state_dict(state, strict=True)
    except Exception:
        # 回退：允许从不完全匹配的 state_dict 加载（仅用于排查/兼容）
        print("严格加载模型权重失败，尝试宽松加载...")
...
def main():
    ...
    parser.add_argument("--ckpt", default=str(Path(__file__).resolve().parent.parent / "gn_cnn_lstm_2task_inst.pt"), help="模型权重路径")
