...
| CNN + Transformer | 全局上下文（自注意力） | 长依赖更稳、关键帧更准、可用预训练 | 计算量较高、需降采样与正则 |

### 五、引入 Transformer（自注意力）

#### 5.1 适用场景与动机

- 长依赖、多段间奏、关键帧位置不固定时，需更强的全局上下文。
- LRC 行级时间戳属于弱标注，边界对齐存在误差，Transformer 更稳。
- 注意力权重可视化便于审阅模型关注点，提升可解释性与可控性。
...
输入：Mel 频谱 (T × F)
 ↓
卷积降维/特征提取（Conv2D/Conv1D）
 ↓
时间位置编码（Positional Encoding）
 ↓
...
- 帧下采样到 100–200 ms token，显著降低序列长度 T。
- 稀疏/分块注意力（Longformer/Transformer-XL）适配分钟级音频。
- 参数建议：`d_model=256–512`、`heads=4–8`、`layers=3–6`、`dropout≈0.2`。
- 优化：AdamW + 线性 warmup + 余弦衰减，`grad_clip=1.0`。

#### 5.4 与本项目的契合（LRC 弱标注）

- 软标签：对齐窗口 ±ΔT 内赋较高权重并线性衰减，增强容忍度。
- 多实例学习（MIL）：以一句歌词为一个 bag，至少一帧为正，适配无精确对齐。
- 注意力热力图：辅助检视间奏与人声边界，提升标注可信度与可解释性。

#### 5.5 与 CNN + LSTM + Attention 的关系

- CNN + LSTM + Attention 更偏局部/轻量的聚焦；Transformer 提供全局自注意力。
- 可作为替换基线或与现有方案做消融对比，常在长序列与边界稳定性上更优。
